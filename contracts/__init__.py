...
class Provider:
    ...
    registered_at: str
    status: str             # "active" | "suspended" | "exited"@allow_storage
...
class Client:
    ...
    registered_at: str@allow_storage
@dataclass
class SLATerms:
    ...
    max_penalty_per_dispute: i32@allow_storage
@dataclass
...
class DisputeEvidence:
    ...
    url: str
    description: str
    submitted_at: str@allow_storage
@dataclass
...
class SLAArbitrator(gl.Contract):# Providers
...
def record_telemetry(
    ...
    # Extract ALL values from storage BEFORE the nondet function
    ...
    # Extract source URLs from storage before nondet
    source_urls_list = []
    ...
    def fetch_telemetry() -> str:
        # Only use captured variables — no self.* reads here
        raw_content = ""
        try:
...
def render_verdict(self, dispute_id: str) -> None:
    ...
    # ─── Extract ALL storage values before nondet ─────────────
    ...
    # Extract evidence from storage before nondet
    evidence_data_list = []
    for eid in evidence_ids[:6]:
        try:
            ...
            evidence_data_list.append((
                role,
                ev.description,     # use description not title
                ev.evidence_type,
                ev.url
            ))
        except:
    ...
    def run_arbitration() -> str:
        # No self.* reads here — only use captured variables
        telemetry_data = ""
...
Penalty scale (max {max_penalty} GEN):
- Minor breach: {penalty} GEN
- Moderate breach: {penalty * 2} GEN
- Severe breach: {max_penalty} GEN
- Latency only: {penalty // 2} GEN

Return ONLY valid JSON:
{{"verdict":"breach_confirmed"|"no_breach"|"partial_breach"|"inconclusive","breach_type":"uptime"|"latency"|"data_freshness"|"error_rate"|"multiple"|"","sla_term_violated":"specific term or empty","measured_uptime":"value or unknown","measured_latency":"value or unknown","measured_error_rate":"value or unknown","reasoning":"4-6 sentences with specific data points","confidence":"high"|"medium"|"low","slash_amount":<int>}}
"""
        result = gl.nondet.exec_prompt(prompt).strip()
        cleaned = result.replace("```json", "").replace("```", "").strip()
        try:
            ...
            return json.dumps({
                ...
                "reasoning": "Could not parse telemetry",
                "confidence": "low",
                "slash_amount": 0
            }, sort_keys=True, separators=(',', ':'))
    ...
    if verdict in ["breach_confirmed", "partial_breach"] and slash_amount > 0:
        ...
        self.providers[respondent].reserved_collateral -= i32(actual_slash)
...
def appeal_verdict(
    ...
    def run_appeal() -> str:
        ...
        prompt = f"""You are reviewing an appeal of an infrastructure SLA breach verdict.Service Endpoint: {endpoint}
SLA Terms: {plain_sla}ORIGINAL BREACH VERDICT: {original.verdict}
ORIGINAL REASONING: {orig_reasoning}
ORIGINAL SLASH AMOUNT: {original.slash_amount} GENPROVIDER'S APPEAL:
{context}ADDITIONAL EVIDENCE:
URL: {add_url if add_url else "None provided"}
Content: {add_content if add_content else "None"}Re-evaluate with fresh eyes. Maintain a high standard for overturning
breach verdicts — the original analysis found a clear SLA breach.
Only overturn if the new evidence definitively shows no breach occurred.Return ONLY valid JSON:
{{
...
"""
            ...
            try:
                ...
                }, sort_keys=True, separators=(',', ':'))    raw = gl.eq_principle.prompt_non_comparative(
        run_appeal,
